...
def get_Promotion(db: Session, PromotionID: int):
    return db.query(models.Promotion).filter(models.Promotion.PromotionID == PromotionID).first()


def get_all_Voucher(db: Session):
    return db.query(models.Voucher).all()

def get_Voucher(db: Session, VidID: int):
    return db.query(models.Voucher).filter(models.Voucher.Vid == VidID).first()