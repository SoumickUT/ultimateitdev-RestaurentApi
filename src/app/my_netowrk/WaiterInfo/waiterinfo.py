...
router = APIRouter(prefix=properties.skill, tags=[properties.waiter_info_tag])
...
@router.get("/waiter_info/")
def waiter_info(db: Session = Depends(get_db)):
    result = main.waiter_info(db)
    if not result:
