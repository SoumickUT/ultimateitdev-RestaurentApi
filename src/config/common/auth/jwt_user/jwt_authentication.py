...
#     user = db.query(models.EsjUser).filter(
#         models.EsjUser.username == request.username).first()
#     if not user:
...
#     user_login_date = db.query(models.EsjUser).filter(models.EsjUser.username == request.username)
#     if user_login_date:
#             user_login_date.update({'last_login_date':datetime.now() + timedelta(hours=+12)})
#     db.add(new_notification)
#     db.commit()
#     user = db.query(models.EsjUser).filter(models.EsjUser.username == request.username).first()
#     return {
